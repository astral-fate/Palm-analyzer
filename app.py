...
def find_model_file(filename):
    """Robustly finds a model file by searching in common model directories."""
    search_paths = [
    ...
    for path in search_paths:
        ...
        if os.path.exists(filepath):
            return filepath
    st.error(f"CRITICAL ERROR: Could not find model file '{filename}'.")
    return None

@st.cache_resource
def load_models():
    """Loads and caches the machine learning models."""
    scaler_path = find_model_file('scaler.joblib')
    kmeans_path = find_model_file('kmeans_model.joblib')
    forecasting_path = find_model_file('forecasting_models.joblib')
...
def load_data():
    """
    Loads data ONLY from the 'Data/' subdirectory to ensure the correct file is used.
    """
    # This path is now specific to prevent loading old/wrong files from other locations.
    data_path = os.path.join(SCRIPT_DIR, 'Data', 'consolidated_palm_farm_data.csv')

    if not os.path.exists(data_path):
        st.error(f"CRITICAL ERROR: Could not find 'consolidated_palm_farm_data.csv' inside the 'Data/' folder.")
        st.stop()
...
def run_forecast(df, forecasting_models, farm_name):
    ...
    """
    # Farm names in the model dictionary might have different casing, so we check case-insensitively
    model_key = next((k for k in forecasting_models.keys() if k.lower() == farm_name.lower()), None)
    
    if not model_key:
        return None, None
    
    model = forecasting_models[model_key]
...
def main():
    ...
    # Handle cases where data might be empty after loading
    ...
    FARM_COORDINATES = {
        'alkatam': [24.4673, 39.6145], 'farm 11': [24.4681, 39.6139],
        'farm 12': [24.4688, 39.6133], 'farm 16': [24.4708, 39.6117],
        'farm 17': [24.4715, 39.6111], 'farm 18': [24.4722, 39.6105],
        'farm 19': [24.4729, 39.6099], 'farm 20': [24.4736, 39.6093],
        'farm 23': [24.4757, 39.6075], 'farm 24': [24.4764, 39.6069]
    }
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "🌟 Performance Overview", "🚨 Anomaly Detection", "📈 NDVI Forecasting",
        "🌱 EDA & Farm Comparison", "📋 Data Explorer"
    ])
    ...
    with tab1:
        ...
        with col1: st.metric("Total Farms Analyzed", len(ALL_FARMS))
        with col2: st.metric("Average Portfolio NDVI", f"{df_historical[NDVI_COL].mean():.3f}")
        with col3: st.metric("Latest Data Point", df_historical[TIMESTAMP_COL].max().strftime('%Y-%m-%d'))
        st.markdown("---")
        ...
        with col1:
            ...
            fig_map = px.scatter_mapbox(farm_coords_df, lat="lat", lon="lon", 
                                      hover_name=FARM_NAME_COL, hover_data=["Performance Tier"],
                                      color="Performance Tier", color_discrete_map=color_map,
                                      zoom=12, height=500, center={"lat": 24.471, "lon": 39.611})
        ...
        with col2:
            ...
            fig_tier = px.bar(tier_counts, x='Performance Tier', y='Count', color='Performance Tier', text_auto=True, color_discrete_map=color_map)
    ...
    with tab3:
        ...
        if forecast_fig:
            ...
            st.error(f"No forecasting model available for **{selected_farm}**. This can happen if the farm name in the data does not exactly match a model file name.")

    # --- TAB 4: EDA & Farm Comparison ---
    with tab4:
        st.subheader("Exploratory Data Analysis")
        farm_stats = df_historical.groupby(FARM_NAME_COL).agg(
            ndvi_mean=(NDVI_COL, 'mean'), ndvi_std=(NDVI_COL, 'std'), ndvi_count=(NDVI_COL, 'count')
        ).reset_index()
